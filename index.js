...
import { token, channelId, serverId, gateway, address } from "./config.mjs";
...
const connectWebSocket = () => {
    ...
    websocket.onmessage = async ({ data }) => {
        ...
        if (op === 10) { websocket.send(JSON.stringify({ op: 2, t: "0.001", d: { op: 0, token, intents: 513 << 0, properties: { os: "raynex7k", browser: "raynex7k", device: "raynex7k" }}})); } 
...
const connectTLS = () => {
  ...
  tlsSocket.on("data", async (data) => {
		...
		tlsSocket.write(`POST /api/v7/channels/${channelId}/messages HTTP/1.1\r\nHost: ${address}\r\nAuthorization: ${token}\r\nContent-Type: application/json\r\nContent-Length: ${Buffer.byteLength(JSON.stringify({ content: `*Vanity Code : \`${vanity}\`\nValue : \`${JSON.stringify(jsonObject)}\`*` }))}\r\n\r\n${JSON.stringify({ content: `*Vanity Code : \`${vanity}\`\nValue : \`${JSON.stringify(jsonObject)}\`*` })}`);
		}
	});
};
